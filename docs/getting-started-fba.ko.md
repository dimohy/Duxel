...
단일 `.cs` 파일로 Duxel 앱을 실행하는 .NET 10 FBA(File-Based App) 기준 가이드입니다. 일반 `net9.0` 프로젝트에서도 Duxel 패키지를 사용할 수 있습니다.
...
| `extended_title_bar_fba.cs` | 앱 탭, 네이티브 캡션 버튼, 드래그 영역, 내장 Win32/DWM 진단을 포함한 Windows 11 스타일 확장 타이틀바 샘플 |
