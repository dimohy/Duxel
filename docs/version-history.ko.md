...
- **[개선]** 첫 콘텐츠 기준 창 표시 — 첫 Vulkan present가 성공할 때까지 네이티브 창을 숨겨 장치·파이프라인·폰트·초기 화면 준비가 빈 창으로 보이지 않도록 하며 렌더러 초기화 자체는 지연하지 않음.
- **[개선]** Vulkan 파이프라인 캐시 영구 저장 — 드라이버 소유 캐시 식별자를 검증하고 opaque 캐시 바이트를 동시 프로세스에서도 원자적으로 저장하며 hit, miss, incompatibility, save, unchanged, unavailable 상태를 시작 진단에 기록.

### 주요 버그 수정

- **[버그]** 콤보 팝업 clipping과 마지막 항목 scrolling — 팝업 그리기·입력·scrollbar geometry를 소유 content 영역으로 제한해 내부 창 하단 padding이 깨끗하게 유지되고 제한된 목록도 마지막 항목을 완전히 볼 때까지 스크롤되도록 수정.

### 배포/릴리스

- **[개선]** 패키지 릴리스 — `Duxel.App`과 `Duxel.Windows.App`을 `0.2.11-preview`로 상향하고 `net8.0`, `net9.0`, `net10.0` 자산을 포함.
...
- **[기능]** 애플리케이션 확장 타이틀바 — `DuxelTitleBarMode.ExtendedContent`, 유효 창 아이콘 조회, 실시간 캡션 버튼 경계, 프레임별 드래그 영역 API를 추가해 탭·검색창·메뉴 등 앱 UI를 창 최상단에 배치하면서 Windows 캡션 명령, Snap Layout, 리사이즈, DPI, 다중 모니터, 최대화 작업영역 동작을 유지.
- **[기능]** .NET 8 패키지 지원 — `Duxel.App`과 `Duxel.Windows.App`에 `net9.0`, `net10.0`과 함께 `net8.0` 자산을 추가하고, 통합 DSL 분석기는 NativeAOT 소비자를 위해 managed `netstandard2.0` 빌드 자산으로 유지.

### 주요 개선 사항

- **[개선]** 확장 캡션 렌더링 — 불투명 Vulkan 클라이언트 surface 위에 라이브러리 소유 최소화·최대화/복원·닫기 아이콘 오버레이를 추가하면서 DWM 히트 코드, 창 명령, 비클라이언트 호버/누름 상태, Windows 11 Snap Layout 동작을 보존.
- **[개선]** 기본 타이틀바 동작성 — Duxel 제목을 왼쪽 정렬하고 실제 창 상태와 최대화/복원 아이콘을 동기화했으며, 유효 네이티브/작업 표시줄 아이콘을 앱 UI에 노출하고 제목·아이콘 우클릭을 표준 Windows 시스템 메뉴로 연결.
- **[개선]** 확장 타이틀바 개발자 문서 — 한·영 구현 가이드, 공개 API 계약, 앱/라이브러리 책임 경계, FBA 사용법, 실제 HWND 검증 절차를 추가.

### 주요 버그 수정

- **[버그]** 미설정 애플리케이션 아이콘 — 암묵적 Duxel 브랜딩을 제거하고 아이콘을 설정하지 않은 경우 네이티브 창, 작업 표시줄, Alt+Tab, 타이틀바 콘텐츠에서 Windows 기본 애플리케이션 아이콘을 일관되게 사용.
- **[버그]** 타이틀바 아이콘 흰색 사각형 — 유효 `HICON`을 WIC로 변환하고 올바른 UV가 있는 이미지 쿼드로 렌더링해 텍스처를 단색 primitive처럼 샘플링하던 문제 수정.
- **[버그]** 보이지 않는 확장 캡션 버튼 — DWM 히트 테스트 성공을 버튼 픽셀 표시로 오판하지 않고 불투명 Vulkan 콘텐츠 위에 캡션 아이콘 픽셀을 표시.
- **[버그]** 복원/최대화 캡션 정렬 불일치 — Duxel 기본 타이틀바와 확장 타이틀바가 같은 캡션 렌더러와 논리 좌표를 사용하도록 통합해 창 상태가 바뀌어도 버튼 높이, 상단 여백, 최대화/복원 아이콘 위치와 형태가 일관되게 유지되도록 수정.

### 배포/릴리스

- **[개선]** 패키지 릴리스 — `Duxel.App`과 `Duxel.Windows.App`을 `0.2.9-preview`로 상향하고 `net8.0`, `net9.0`, `net10.0` 자산을 포함.
