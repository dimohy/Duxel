param(
    [string]$SamplePath = "samples/fba/vector_primitives_bench_fba.cs",
    [string]$LegacyOut = "artifacts/vector-bench-legacy.json",
    [string]$OptimizedOut = "artifacts/vector-bench-optimized.json",
    [double]$PhaseSeconds = 1.0,
    [string]$PrimitiveCounts = "6000,12000,24000",
    [switch]$Managed,
...
try {
    ...
    function Invoke-BenchRun {
        ...
        $prevPhase = $env:DUXEL_VECTOR_BENCH_PHASE_SECONDS
        $prevCounts = $env:DUXEL_VECTOR_BENCH_COUNTS
        $prevOut = $env:DUXEL_VECTOR_BENCH_OUT

        try {
            $env:DUXEL_VK_LEGACY_CLIP_CLAMP = $LegacyFlag
            $env:DUXEL_VECTOR_BENCH_PHASE_SECONDS = [string]::Format([System.Globalization.CultureInfo]::InvariantCulture, "{0:0.###}", $PhaseSeconds)
            $env:DUXEL_VECTOR_BENCH_COUNTS = $PrimitiveCounts
            $env:DUXEL_VECTOR_BENCH_OUT = $fullOut
            ...
            if ($null -eq $prevPhase) { Remove-Item Env:DUXEL_VECTOR_BENCH_PHASE_SECONDS -ErrorAction SilentlyContinue } else { $env:DUXEL_VECTOR_BENCH_PHASE_SECONDS = $prevPhase }
            if ($null -eq $prevCounts) { Remove-Item Env:DUXEL_VECTOR_BENCH_COUNTS -ErrorAction SilentlyContinue } else { $env:DUXEL_VECTOR_BENCH_COUNTS = $prevCounts }
            if ($null -eq $prevOut) { Remove-Item Env:DUXEL_VECTOR_BENCH_OUT -ErrorAction SilentlyContinue } else { $env:DUXEL_VECTOR_BENCH_OUT = $prevOut }
        }
    }
    ...
    $rows = for ($i = 0; $i -lt $legacy.records.Count; $i++) {
        ...
        [PSCustomObject]@{
            primitives = [int]$left.primitives
